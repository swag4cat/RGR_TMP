...
async def assign_operator(
    ...
    # Проверяем, что пользователь существует и он оператор
    ...
    await log_action(
        ...
        details={"operator_id": user_id, "object_id": object_id},
        request=request
    )
    
    return {"message": f"Operator {user.username} assigned to {obj.name}"}
